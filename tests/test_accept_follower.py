...
from commands.accept_follower import AcceptFollowerCommand
...
def mock_oauth_post(url, headers=None, data=None):
    if url == ACTOR['endpoints']['proxyUrl']:
        ...
        added_data = {'id': ACCEPT_ID, 'published': '2020-01-01T00:00:00Z', 'actor': ACTOR_ID}
        return MagicMock(status_code=200, json=lambda: {**json.loads(data), **added_data})
    else:
        return MagicMock(status_code=404)

class TestAcceptFollowerCommand(unittest.TestCase):
    ...
    def test_accept_follower_remote(self, mock_requests_get, mock_requests_post, mock_file):
    ...
    @patch('requests_oauthlib.OAuth2Session.post', side_effect=mock_oauth_post)
    @patch('requests_oauthlib.OAuth2Session.get', side_effect=mock_oauth_get)
    def test_accept_follower_local(self, mock_requests_get, mock_requests_post, mock_file):
